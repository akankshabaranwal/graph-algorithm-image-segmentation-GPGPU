	...
	if(	d_edge_flag[tid]==1)
		{
			...
			atomicMax(d_vertex_list_size,(u+1)); //how can max(v) be > max(u), error!!!!!
			}
		}
	}
}
...
__global__ void MakeFlagForVertexList(int *d_pick_array, unsigned int *d_edge_flag, int no_of_edges)
{
	...
	if(tid>0)
	{
		if(d_pick_array[tid]>d_pick_array[tid-1])//This line may be causing problems
		{
			d_edge_flag[tid]=1;
		}
	}
