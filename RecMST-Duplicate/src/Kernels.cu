...
__global__ void CreateLevelOutput(unsigned char *d_output_image, unsigned char *d_component_colours, unsigned int* d_level, unsigned int* d_prev_level_component, unsigned int no_of_rows, unsigned int no_of_cols) 
{
	unsigned int row = blockDim.x * blockIdx.x + threadIdx.x;
    unsigned int col = blockDim.y * blockIdx.y + threadIdx.y;
