...
# - Split primitive: I think there should be more efficient ways rather than just sorting.

from graphvisualise import GraphVisualization
...
def MST_recursive(V, E, W, orig_edge, MST):
    ...
    # 9.2 Create flag, first element not flagged so can use simple sum for scan
    ...
    for i in range(0, len(E)):  # in parallel for all edges
        ...
        if supervertexid_u == supervertexid_v:
            E[i] = math.inf  # Mark edge for removal, inf so at back after splitting
    ...
    for i in range(0, len(E)):  # in parallel for all edges
        ...
        id_v = E[i]
        edge_weight = W[i]
        if id_u != math.inf and id_v != math.inf:
            supervertexid_u = supervertex_ids[id_u]
            supervertexid_v = supervertex_ids[id_v]
            UVW.append((supervertexid_u, supervertexid_v, edge_weight))
        else:
            UVW.append((math.inf, math.inf, edge_weight))
    ...
    for i in range(1, len(UVW)):  # in parallel for all edges
        ...
        if supervertexid_u != math.inf and supervertexid_v != math.inf:
            if prev_supervertexid_u != supervertexid_u and prev_supervertexid_v != supervertexid_v:
                F3[i] = 1
        else:
            new_edge_size = i
            break
    ...
    for i in range(0, new_edge_size):  # In parallel for all edges (can use new edge size)
        if F3[i]:
            ...
            if supervertex_id_u != math.inf and supervertex_id_v != math.inf:  # Shouldn't be necessary I think, TODO check
