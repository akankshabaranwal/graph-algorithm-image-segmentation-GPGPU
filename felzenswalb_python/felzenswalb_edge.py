...
# TODO: experiment if using L2 instead of L1 also produces good result
# TODO: experiment with other fast edge detectors, such as a simple sobel operator
def felzenswalb_edge(input_image, edge_image, sigma, k, min_cmp_size):
...
def segment_fastmst_hierarchies(V, E, color_term, edge_strength):
    hierarchy = []
    ...
    while (len(V)) > 1:
        V, E, color_term, edge_strength, supervertex_ids = MST_recursive(V, E, color_term, edge_strength)
...
def MST_recursive(V, E, color_term, edge_strength):
    # A. Find minimum weighted edge
    # - - - - - - - - - - - - - - -

    # 0. NEW! Calculate weight based on color and edge term
    W = [c * e for c, e in zip(color_term, edge_strength)]
    ...
    return new_V, new_E, new_color_term, new_edge_strength, supervertex_ids
...
def create_graph_4_neigbour(image, edge_image):
    ...
    return (vertices, edges, weights, edge_strength)